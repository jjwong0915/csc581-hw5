...
int main() {
  ...
  while (window.isOpen()) {
    ...
    if (world_socket.recv(world_data)) {
      ...
      sf::RectangleShape main_character{{30, 30}};
      main_character.setPosition(world_object.main_character.x,
                                 world_object.main_character.y);
      window.draw(main_character);
      for (const auto& o : world_object.obstacles) {
        sf::RectangleShape obstacle_shape({o.w, o.h});
        obstacle_shape.setPosition(o.x, o.y);
        obstacle_shape.setFillColor(sf::Color::Blue);
